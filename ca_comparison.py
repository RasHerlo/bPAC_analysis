...
def create_trace_overview_bpac_spont(df, output_path):
    ...
    Args:
        ...
        output_path (str): Path to save the plot
    """
    # Filter out traces that are not numpy arrays
    valid_traces = [idx for idx, row in df.iterrows() if isinstance(row['ChanB_Z_trc'], np.ndarray)]
...
def create_trace_overview_ap_conditions(df, output_path):
    ...
    valid_traces = [idx for idx, row in df.iterrows() 
                   if isinstance(row['ChanB_Z_trc'], np.ndarray) and not pd.isna(row['Stim'])]
...
def main():
    ...
    create_trace_overview_bpac_spont(bpac_spont_df, 
                                   os.path.join(output_dir, 'bpac_spont_traces.pdf'))
    create_trace_overview_ap_conditions(bpac_aps_df, 
                                      os.path.join(output_dir, 'ap_conditions_traces.pdf'))
    
    # Extract values for bar plot using only Z-scored traces
    print("\nProcessing Z-scored traces for bar plot...")
    bpac_spont_values = extract_max_values(bpac_spont_df, 'bPAC_spont', 'ChanB_Z_trc')
    ap_clean_values = extract_max_values(bpac_aps_df, 'AP_clean', 'ChanB_Z_trc')
    bpac_ap_values = extract_max_values(bpac_aps_df, 'bPAC_AP', 'ChanB_Z_trc')
    
    # Create and save the barplot
    output_path = os.path.join(output_dir, 'calcium_comparison.pdf')
    create_barplot(bpac_spont_values, ap_clean_values, bpac_ap_values, 'ChanB_Z_trc', output_path)
